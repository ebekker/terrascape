...
{
    public static class HasReadResource
    {
        public interface Skill<T>
        {
            Result<T> Read(Input<T> input);
        }

        public class Input<TResource>
        {
            public TResource CurrentState { get; set; }
        ...
        public static bool HasReadResourceSkill(this Type targetType, Type resType) =>
            targetType.HasSkill(typeof(Skill<>), resType);

        public static void InvokeReadResourceSkill(
